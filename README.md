...
The terminal view runs full screen, like Claude Code's fullscreen mode. Each prompt starts at the top of the screen with its reply below, and the input stays pinned to the bottom. Scroll back with the mouse wheel or PgUp/PgDn, and typing jumps back to the bottom. Drag to select text, and letting go copies it (pbcopy on a Mac, OSC 52 elsewhere). Double-click copies a word, dragging past the top or bottom scrolls, and a click or a keypress clears the selection. On exit the whole transcript prints to the normal screen, where your terminal's scrollback has it. `HARNESS_FULLSCREEN=0` keeps a plain line prompt instead.
