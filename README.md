...
- `src/diff.ts` + `src/render.ts` draw the GitHub-style diff. `src/markdown.ts` styles streamed replies. `src/keys.ts` decodes modified keys like Shift+Enter before readline sees them.
